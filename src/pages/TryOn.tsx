...
const FOOT_ICON = (
  ...
  >
    ...
    />
    <path
      d="M13 16c1.8 0 3 .5 5 .5s3.5-.5 4.5.8-.3 2.7-2 2.7H10"
      stroke="currentColor"
      strokeWidth="1.6"
      strokeLinecap="round"
      strokeLinejoin="round"
...
export function TryOn(_props: TryOnProps) {
  const { shoeId } = useParams();
  const navigate = useNavigate();

  const initialShoe = useMemo(
    () => getShoeById(shoeId ?? "") ?? shoes[0],
    [shoeId]
  );
  ...
  const [calibrationOverrides, setCalibrationOverrides] =
    useState<Record<string, ShoeCalibration>>({});

  // `?debug=true` unlocks the full AR diagnostic panel.
  ...
  const canShowDevTools =
    import.meta.env.DEV || debugQueryEnabled;

  const [showDebugPanel, setShowDebugPanel] =
    useState(debugQueryEnabled);

  const [modelTestMode, setModelTestMode] =
    useState(false);

  const [forceHideModel, setForceHideModel] =
    useState(false);
  ...
  const [helpModalOpen, setHelpModalOpen] =
    useState(false);

  const [helpDontShowAgain, setHelpDontShowAgain] =
    useState(() => {
      try {
        return (
          localStorage.getItem("arHelpDontShowAgain") ===
          "true"
        );
      } catch {
        return false;
  ...
  const shoeIndex = shoes.findIndex(
    (s) => s.id === selectedShoe.id
  );
  ...
  const tracking = useARTracking(
    ...
    videoWrapperRef,
    screenState === "live" && camera.isReady,
    mirrored
  );

  const shoeModelUi = useShoeModel(
    screenState === "live"
      ? effectiveShoe.model
      : null
  );
  ...
  const hasManualAdjustment =
    selectedShoe.id in calibrationOverrides;

  const liveViewport =
    computeViewportPlane(containerAspect);

  const applyAdjustment = useCallback(
    (next: Partial<ShoeCalibration>) => {
      setCalibrationOverrides((prev) => {
        const base: ShoeCalibration = {
          ...selectedShoe,
          ...prev[selectedShoe.id],
        };
  ...
  const dragScale = useDragScaleGesture({
    ...
    worldUnitsPerPixelX:
      wrapperRect && wrapperRect.width > 0
        ? liveViewport.width / wrapperRect.width
        : 0,

    worldUnitsPerPixelY:
      wrapperRect && wrapperRect.height > 0
        ? liveViewport.height / wrapperRect.height
        : 0,
  });
    ...
    (deltaDeg: number) => {
      setCalibrationOverrides((prev) => {
        ...
        return {
          ...
          [selectedShoe.id]: {
            ...base,
            rotationZ: base.rotationZ + deltaDeg,
          },
        };
      });
    },
      ...
      setCalibrationOverrides((prev) => {
        ...
        return {
          ...
          [selectedShoe.id]: {
            ...
            scale: Math.min(
              4,
              Math.max(0.2, base.scale * factor)
            ),
          },
        };
      });
  ...
  // Preload next shoe model.
  useEffect(() => {
    const next =
      shoes[(shoeIndex + 1) % shoes.length];

    if (next) {
      preloadShoeModel(next.model);
    }
  }, [shoeIndex]);
  ...
  const handleFileChange = useCallback(
    async (
      ...
      if (!validation.valid) {
        setUploadError(
          validation.reason ?? "Invalid file."
        );
        return;
      }
  ...
  if (screenState === "unsupported-live") {
    return (
      <div className="flex h-dvh w-full flex-col bg-neutral-950 text-white">
        ...
        {uploadError && (
          <p className="px-6 pb-4 text-center text-xs text-red-400">
            {uploadError}
          </p>
        )}
      ...
      : shoeModelUi.loading
        ...
        : tracking.status === "idle" ||
          ...
          ? {
              tone: "neutral" as const,
              title:
                "Point your camera at your feet",
              subtitle: undefined,
              icon: FOOT_ICON,
            }
          : tracking.status ===
            ...
            ? {
                tone: "neutral" as const,
                title:
                  "Place your foot inside the frame",
                subtitle:
                  "Make sure your feet are clearly visible and well lit.",
                icon: FOOT_ICON,
              }
            : null;

  const canUseThisLook =
    camera.isReady &&
    !shoeModelUi.loading &&
    !shoeHasError &&
    !trackingFatal;
  ...
  return (
    <div className="flex h-dvh w-full flex-col bg-black">
      <div
        ref={videoWrapperRef}
        className="relative isolate flex-1 touch-none select-none overflow-hidden bg-black"
        {...dragScale.handlers}
      >
        {/* Camera layer */}
        <CameraView
        ...
        {trackingFatal && (
          <div className="absolute inset-0 z-30 overflow-y-auto bg-black/90">
            <ErrorState
              icon="warning"
      ...
      {uploadError && (
        <p className="bg-black px-6 py-3 text-center text-xs text-red-400">
          {uploadError}
        </p>
      )}
